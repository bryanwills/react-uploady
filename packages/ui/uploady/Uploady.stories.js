...
const ContextUploadButtonWithPrepareHooks = (
    props: ?ContextButtonWithHooksProps,
) => {
    console.log(
        "Rendering Context Upload Button with Prepare Hooks (pre-send & batch-start)",
    );
...
const ProcessPending = ({
    ...
    return (
        <button id={id} onClick={() => processPending(options)}>
            {title}
        </button>
    );
};
...
const ClearPending = () => {
    ...
    return (
        <button id="clear-pending" onClick={clearPending}>
            CLEAR PENDING
        </button>
    );
};
...
const QueueItem = ({ item }: { item: BatchItem }) => {
    ...
    return (
        <li data-test="queue-item">
            <span>NAME: {item.file.name}</span>
            <br/>
            <span>
        STATE:
        <span
            data-test="queue-item-state"
...
const UploadFormWithInternalInput = () => {
    ...
    const onSelectChange = useCallback(
        (e: SyntheticKeyboardEvent<HTMLSelectElement>) => {
...
const renderUploadyFromBundle = () => {
    ...
    try {
        ...
        result = react.createElement(rpldy.Uploady, uploadyProps, [
            react.createElement(MyUploadButton),
        ]);
    } catch (ex) {
        result = react.createElement("p", {
            style: { color: "red" },
            children: `ERROR !!! ${ex.message}`,
        });
    }
...
const renderUploadyAll = () => {
    ...
    return react.createElement(rpldy.uploady.Uploady, uploadyProps, [
        uploadButton,
        uploadPreview,
    ]);
};
