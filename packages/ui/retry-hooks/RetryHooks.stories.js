...
const RetryUi = () => {
    ...
    const onRetryBatch = useCallback(
        (e: SyntheticMouseEvent<HTMLLIElement>) => {
    ...
    return (
        <>
            ...
            <br/>
            <button id="retry-all" onClick={onRetryAll}>
                Retry All
            </button>
...
export const WithRetry: UploadyStory = createUploadyStory(
    ({ destination, enhancer, multiple, grouped, groupSize }): Node => {
        return (
            <Uploady
                debug
                multiple={multiple}
                destination={destination}
                enhancer={enhancer ? composeEnhancers(retryEnhancer, enhancer) : retryEnhancer}
                grouped={grouped}
...
const PreviewItemContainer = styled.article`
    ...
    box-shadow: ${({ state }: { state: string }) =>
        state ? STATE_COLORS[state] : "#c3d2dd"} 0px 8px 5px -2px;
    position: relative;
    align-items: center;
...
const PreviewItemBar = styled.div`
    ...
    padding: 10px 0;
    width: 100%;
    box-shadow: #5dbdec 0 -3px 2px -2px;
`;
...
const ClearPreviewsButton = ({
                                 methods,
                                 previews,
                             }: {
    methods: RefObject<PreviewMethods>,
...
const QueueItem = memo((props: { id: string, name: string, url: string }) => {
    ...
    return (
        <PreviewItemContainer
            state={itemState}
            data-test="preview-item-container"
...
export const WithRetryAndPreview: UploadyStory = createUploadyStory(
    ({ destination, enhancer, multiple, grouped, groupSize }): Node => {
        return (
            <Uploady
                debug
